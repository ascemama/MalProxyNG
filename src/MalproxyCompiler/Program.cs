...
namespace MalproxyCompiler
{
    ...
    class FunctionCodeGenerator
    {
        ...
        public string GenerateHomeStubCode(bool isDLL=false)
        {
            StringBuilder function_code = new StringBuilder();
            string return_type = (_return_type != null) ? _return_type.ParameterTypeCast : "void";
            if (isDLL){
                function_code.AppendLine($@"DllExport {return_type} WINAPI {_function_name}_stub (");
            }
            else{
    ...
    class CodeGenerator
    {
        private string GenerateHomeCode(IEnumerable<FunctionCodeGenerator> functions, bool isDLL=false)
        {
            ...
            if (isDLL)
            {
                code_builder.AppendLine($"#define DllExport extern \"C\"   __declspec( dllexport )");
                code_builder.AppendLine();
            }
            ...
            foreach (var func in functions)
            {
                ...
                code_builder.AppendLine(func.GenerateHomeStubCode(isDLL));
            }
        ...
        public void GenerateHomeCodeDLL(string output_path, IEnumerable<FunctionCodeGenerator> functions)
        {
            GenerateHomeCode(output_path, functions, true);
        }

        public void GenerateHomeCode(string output_path, IEnumerable<FunctionCodeGenerator> functions,bool isDLL=false)
        {
            string code = GenerateHomeCode(functions,isDLL);
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            code.GenerateHomeCodeDLL(System.IO.Path.Combine(target, "MalproxyClientDLL", "autogenerated.home.DLL.cpp"), functions);
